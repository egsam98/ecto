...
recursive/internal processing\
(integers, booleans, strings, etc.).
    : Error format: 
...
Optional Schema
    : A composite wrapper over any schema.\
The inner schema is applied only when data is present (not null).
Requires a pointer type.
Error format is identical to *Atomic*.
...
- Common for all types:
  - Required
    : Value must not be zero-value. If this test fails, all subsequent tests are skipped.\
    Error: `required`
...
- For integers
  - Eq
    : Value must equal the specified integer\
